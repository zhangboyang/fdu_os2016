...
void arch_init_pmm_zone()
{
    ...
    for (int i = 0; i < MAX_MEMORY_ZONE; i++) {
        kprintf(" zone %10s base %016llx size %016llx end %016llx pagesz %x\n", zone_name[i], pmm_zone[i].base, pmm_zone[i].size, pmm_zone[i].base + pmm_zone[i].size, pmm_zone[i].page_size);
    }
}
